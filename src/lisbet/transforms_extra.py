...
GaussianJitter
    Apply Gaussian jitter with per-element Bernoulli sampling across (time, keypoints,
    individuals). Adds N(0, sigma^2) noise broadcast across space dimensions, with
    coordinates clamped to [0, 1]. Simulates sporadic tracking noise.

GaussianBlockJitter
    Apply Gaussian jitter within element-specific temporal blocks. Each selected
    (time, keypoint, individual) element triggers a block of frames receiving
    N(0, sigma^2) noise for that specific keypoint and individual.
